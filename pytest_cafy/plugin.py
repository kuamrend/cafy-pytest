    ...
    if config.option.enable_live_update:
        ...
        try:
            if CafyLog.registration_id:
                ...
                response = requests.patch(url, json=params, headers=headers, timeout=60)
                if response.status_code == 200:
        ...
        try:
            ...
            response = requests.post(url, json=CafyLog.collected_testcases, headers=headers, timeout=60)
            if response.status_code == 200:
